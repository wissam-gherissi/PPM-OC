...
fold3_t4= list(itemgetter(*idx3)(nb_itemseqs))
...
model = load_model('output_files/models/model_orders_filtered_12-1.02.h5')


# define helper functions
def encode(sentence, times, times3, nb_items, maxlen=maxlen):
    ...
    for t, char in enumerate(sentence):
        ...
        X[0, t + leftpad, len(chars) + 5] = nb_items[t]
...
# make predictions
with open('output_files/results/next_activity_and_time_%s.csv' % eventlog, 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    spamwriter.writerow(
        ["CaseID", "Prefix length", "Ground truth", "Predicted", "Levenshtein",
         "Damerau", "Jaccard", "Ground truth times", "Predicted times", "Ground truth TR", "Predicted TR", "RMSE", "MAE"])
    for prefix_size in range(2, maxlen):
        print(prefix_size)
        for line, caseid, Ptimes, Ptimes3, nb_items, PtimesF in zip(lines, caseids, lines_t, lines_t3, lines_t4, lines_t5):
            Ptimes.append(0)
            cropped_line = ''.join(line[:prefix_size])
            cropped_times = Ptimes[:prefix_size]
            cropped_times3 = Ptimes3[:prefix_size]
            cropped_times4 = nb_items[:prefix_size]

            if '!' in cropped_line:
                continue  # make no prediction for this case, since this case has ended already
            ground_truth = ''.join(line[prefix_size:prefix_size + predict_size])
            ground_truth_t = Ptimes[prefix_size:prefix_size + predict_size]
            ground_truth_tr = PtimesF[prefix_size:prefix_size + predict_size]
            predicted = ''
            predicted_t = []
            predicted_tr = []
            ...
            if len(ground_truth) > 0:
                ...
                output.append('; '.join(str(x) for x in ground_truth_tr))
                output.append('; '.join(str(x) for x in predicted_tr))
                if len(predicted_t) > len(
                    ...
                    predicted_tr = predicted_tr[:len(ground_truth_tr)]
                if len(ground_truth_t) > len(
                    ...
                    predicted_tr.extend(range(len(ground_truth_tr) - len(predicted_tr)))
                if len(ground_truth_t) > 0 and len(predicted_t) > 0:
                    output.append('')
                    output.append(metrics.mean_absolute_error([ground_truth_t[0]], [predicted_t[0]]))
                    output.append(metrics.mean_absolute_error([ground_truth_tr[0]], [predicted_tr[0]]))
